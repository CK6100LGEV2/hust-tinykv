...
func (rn *RawNode) HasReady() bool {
	...
	if !r.softState().isSoftStateEqual(rn.prevSoftSt) || !IsEmptySnap(r.RaftLog.pendingSnapshot) ||
		len(r.msgs) > 0 || len(r.RaftLog.unstableEntries()) > 0 || len(r.RaftLog.nextEnts()) > 0 {
		return true
	}
	return false
}
